...
async def successful_payment(message: Message):
    ...
    await dp.bot.send_message(
        ...
        f"Платеж на сумму {message.successful_payment.total_amount /
                           100} {message.successful_payment.currency} прошел успешно!!!",
    )
...
async def cmd_order(message: Message):
    ...
    builder.add(InlineKeyboardButton(text="Оформить заказ", callback_data="order"))
    await message.answer(
        "После нажатия на кнопку 'Оформить заказ' будет создана заявка. Все заявки обрабатываются вручную админом. После того как заявка будет обработана, я пришлю вам информацию для оплаты.",
    )
